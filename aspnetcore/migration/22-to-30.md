...
* Fügen Sie einen Paket Verweis auf [Microsoft. aspnetcore. MVC. newtonsoft JSON](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)hinzu.
...
Das Endpunkt Routing unterstützt die gleiche Routing Muster Syntax und die Funktionen zum `IRouter`Erstellen von Routen Mustern wie. Das Endpunkt Routing `IRouteContraint`unterstützt. Das Endpunkt Routing `[Route]`unter `[HttpGet]`stützt, und die anderen MVC-Routing Attribute.
...
* Wenn die APP Authentifizierungs-/Autorisierungsfeatures `AuthorizePage` wie `[Authorize]`oder verwendet, platzieren Sie `UseAuthentication` den `UseAuthorization` -Befehl nach und **nach** `UseRouting`.
* Wenn die APP [cors](xref:security/cors) -Funktionen wie z `[EnableCors]`. b. verwendet, platzieren `UseCors` Sie als nächstes.
...
Wenn die APP `AuthorizeFilter` als globaler Filter in MVC verwendet, empfiehlt es sich, den Code so zu umgestalten, dass eine Richt `UseAuthorization` Linie für die Middleware bereitgestellt wird.

Im folgenden Beispiel wird eine benutzerdefinierte Richtlinie auf alle Anforderungen angewendet, wenn `UseAuthorization` aufgerufen wird, und die `HomeController` ermöglicht den Zugriff, ohne dass der Benutzer sich bei der App anmeldet:
...
Im folgenden Beispiel wird `UseAuthorization` die Autorisierung ohne eine Standard Richtlinie verarbeitet, aber der `/healthz` Integritätsprüfung-Endpunkt erfordert, dass der `admin` Benutzer in der Rolle ist: 
