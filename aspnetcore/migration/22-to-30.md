...
Das Validierungssystem in .net Core 3,0 und höher behandelt Parameter, die keine NULL-Werte zulassen, oder gebundene Eigenschaften so, als hätten Sie ein `[Required]` Attribut. Weitere Informationen finden Sie unter [[Required]-Attribut](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
