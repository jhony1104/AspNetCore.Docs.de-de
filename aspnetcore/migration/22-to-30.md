...
Wenn Ihre APP docker verwendet, verwenden Sie ein Basis Image, das ASP.net Core 3,0 enthält. Beispiel: `docker pull mcr.microsoft.com/dotnet/core/aspnet:3.0`Hyper-V-Hosts oder Hyper-V-Hostcluster in einem separaten Namespace als verwaltete Hyper-V-Hosts hinzuzufügen.
...
* MVC-`Newtonsoft.Json` Unterstützung &ndash; Unterstützung für die Verwendung von MVC mit `Newtonsoft.Json` ist nun Bestandteil von [Microsoft. aspnetcore. MVC. newtonsoftware](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
* `IHostingEnvironment` `IWebHostEnvironment`finden Sie in [dieser GitHub-Ankündigung](https://github.com/aspnet/AspNetCore/issues/7749).
...
Ihre APP erfordert möglicherweise `Newtonsoft.Json` Integration, wenn Sie `Newtonsoft.Json`spezifische Funktion wie jsonpatch oder Konverter verwendet oder `Newtonsoft.Json`spezifische Typen [formatiert](xref:web-api/advanced/formatting) .

Informationen zum Verwenden von JSON.net in einem ASP.net Core 3,0 SignalR-Projekt finden Sie in diesem Dokument unter [Wechseln zu "newtonsoft. JSON](#switch-to-newtonsoftjson) ".
...
* Aktualisieren Sie `Startup.ConfigureServices`, um `AddNewtonsoftJson`aufzurufen.
  ...
  * `AddControllers`
  ...
  JSON.NET-Einstellungen können im `AddNewtonsoftJson`-Aufrufsatz festgelegt werden:
...
Wenn die APP eine `AuthorizeFilter` als globaler Filter in MVC verwendet, empfiehlt es sich, den Code so zu umgestalten, dass eine Richtlinie im `AddAuthorization`aufgerufen wird.
...
Richtlinien können auch angepasst werden. Nach dem vorherigen Beispiel wird der `DefaultPolicy` so konfiguriert, dass er Authentifizierung und einen bestimmten Bereich erfordert:
...
* Die Link Generierung erforderte die Angabe des `Async` Suffix. Zum Beispiel:
...
* Bei der Link Generierung muss das `Async` Suffix nicht angegeben werden. Zum Beispiel:
...
1. Installieren Sie [Microsoft. aspnetcore.SignalR. Das nuget-Paket "Protokolls. newtonsoft JSON](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) ".
...
1. Verketten Sie auf dem-Server einen `AddNewtonsoftJsonProtocol` Methodenaufrufe des `AddSignalR`-Methoden Aufrufes in `Startup.ConfigureServices`:
