...
Wenn Sie sich auf eine [Global. JSON](/dotnet/core/tools/global-json) -Datei stützen, um eine bestimmte .net Core SDK Version als `version` Ziel festzustellen, aktualisieren Sie die-Eigenschaft auf die installierte Version von .net 5,0 SDK. Beispiel:
...
Aktualisieren Sie in der Projektdatei die `Microsoft.AspNetCore.*` einzelnen `Microsoft.Extensions.*` Attribute und das `Version` Attribut des Paket Verweises auf 5.0.0 oder höher. Beispiel:
...
Verwenden Sie für apps, die Docker verwenden, ein Basis Image, das die ASP.net Core 5,0-Laufzeit enthält. Beispiel:
