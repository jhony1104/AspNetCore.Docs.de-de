...
* Ändern Sie das Ziel Framework in .net Core 2,1, indem Sie die Projekt `<TargetFramework>netcoreapp2.1</TargetFramework>`Datei in aktualisieren.
* Ersetzen Sie den Paket Verweis `Microsoft.AspNetCore.All` für durch einen Paket Verweis `Microsoft.AspNetCore.App`für. Möglicherweise müssen Sie Abhängigkeiten hinzufügen, die aus `Microsoft.AspNetCore.All`entfernt wurden. Weitere Informationen finden Sie unter <xref:fundamentals/metapackage#migrate> und <xref:fundamentals/metapackage-app>.
* Entfernen Sie das "Version"-Attribut für den Paket `Microsoft.AspNetCore.App`Verweis auf. Bei Projekten, `<Project Sdk="Microsoft.NET.Sdk.Web">` die verwenden, muss die-Version nicht festgelegt werden. Die Version wird vom Ziel Framework impliziert und so ausgewählt, dass Sie am besten mit der Funktionsweise von ASP.net Core 2,1 kompatibel ist. Weitere Informationen finden Sie unter den [Regeln für Projekte, die auf das freigegebene Framework abzielen](#rules-for-projects-targeting-the-shared-framework) .
...
* Optional: Sie können das ** &lt;dotnetclitoolreference&gt; ** -Element für `Microsoft.VisualStudio.Web.CodeGeneration.Tools`entfernen. Sie können dieses Tool durch Ausführen `dotnet tool install -g dotnet-aspnet-codegenerator`von durch eine Global installierte Version ersetzen.
* Für 2,1 ist eine [Razor-Klassenbibliothek](xref:razor-pages/ui-class) die empfohlene Lösung für die Verteilung von Razor-Dateien. Wenn Ihre APP eingebettete Sichten verwendet oder anderweitig von der Runtime-Kompilierung von Razor- `<CopyRefAssembliesToPublishDirectory>true</CopyRefAssembliesToPublishDirectory>` Dateien abhängig `<PropertyGroup>` ist, fügen Sie einer in der Projektdatei hinzu.
...
* Alle Projekte, die oder `Microsoft.AspNetCore.All` `Microsoft.AspNetCore.App` verwenden müssen, müssen einen Paket Verweis für das Paket in der Projektdatei hinzufügen. Dies ist auch dann der Fall, wenn `Microsoft.AspNetCore.All` Sie `Microsoft.AspNetCore.App`einen Projekt Verweis auf ein anderes Projekt mit oder enthalten.

  Beispiel:

  * `MyApp`verfügt über einen Paket Verweis `Microsoft.AspNetCore.App`auf.
  * `MyApp.Tests`weist einen Projekt Verweis auf `MyApp.csproj`auf.

  Fügen Sie einen Paket Verweis `Microsoft.AspNetCore.App` für `MyApp.Tests`hinzu. Weitere Informationen finden Sie unter [Integrationstests sind schwer einzurichten und können bei der Wartung von freigegebenen Frameworks unterbrechen](https://github.com/dotnet/aspnetcore/issues/3156).
...
Ändern Sie `FROM` die Zeilen in der *dockerfile-Datei* so, dass die neuen Bildnamen und-Tags in der 2,1-Spalte der vorangehenden Tabelle verwendet werden. Weitere Informationen finden Sie unter [Migrieren von aspnetcore-docker-Repositorys zu dotnet](https://github.com/aspnet/Announcements/issues/298).
...
Der neue `Main` ersetzt den-Befehl `BuildWebHost` durch ["](/dotnet/api/microsoft.aspnetcore.mvc.testing.webapplicationfactory-1.createwebhostbuilder)" mit "". [Iwebhostbuilder](/dotnet/api/microsoft.aspnetcore.hosting.iwebhostbuilder) wurde zur Unterstützung einer neuen [Infrastruktur für Integrationstests](xref:test/integration-tests)hinzugefügt.

### <a name="changes-to-startup"></a>Änderungen am Start

Der folgende Code zeigt die Änderungen an 2,1-Vorlagen generiertem Code. Alle Änderungen sind neu hinzugefügter Code, `UseBrowserLink` mit dem Unterschied, dass entfernt wurde:
...
* Alle Fehlerbehebungen oder neuen Features für die Identität sind im [Metapaket Microsoft. aspnetcore. app](xref:fundamentals/metapackage-app)enthalten. Sie erhalten automatisch die aktualisierte Identität, `Microsoft.AspNetCore.App` wenn aktualisiert wird.
...
* Verwenden Sie Identity den Code der Benutzeroberfläche 2,0 ohne Änderungen. Der Identity Code der Benutzeroberfläche 2,0 wird vollständig unterstützt. Dies ist ein guter Ansatz, wenn an dem generierten Identity Code bedeutende Änderungen vorgenommen wurden.
* Löschen Sie Ihren Identity vorhandenen 2,0-Code, und [Gerüst Identity ](xref:security/authentication/scaffold-identity) Sie in Ihr Projekt auf. Das Projekt verwendet die [ Identity ASP.net Core](xref:security/authentication/identity) [ Razor -Klassenbibliothek](xref:razor-pages/ui-class). Sie können Code und die Benutzeroberfläche für jeden Benutzer Identity Oberflächen Code generieren, den Sie geändert haben. Wenden Sie die Codeänderungen auf den neu gerüdierten UI-Code an.
* Löschen Sie den Identity vorhandenen [2,0-Code, Identity ](xref:security/authentication/scaffold-identity) und setzen Sie sich mit der Option zum über **Schreiben aller Dateien**in das Projekt um.

### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Ersetzen Identity von 2,0 UI durch Identity die Razor 2,1-Klassenbibliothek

In diesem Abschnitt werden die Schritte zum Ersetzen des generierten Identity Codes der ASP.net Core 2,0-Vorlage durch die [ASP.net Core Identity ](xref:security/authentication/identity) [ Razor -Klassenbibliothek](xref:razor-pages/ui-class)beschrieben. Die folgenden Schritte gelten für ein Razor Pages-Projekt, aber der Ansatz für ein MVC-Projekt ist ähnlich.
...
* Gerüst für Ihr Projekt: [ Identity ](xref:security/authentication/scaffold-identity)
  * Wählen Sie die Projekte aus, die die Datei * \_Layout. cshtml* beenden.
...
* Löschen Sie Identity die vom Gerüst `IdentityDbContext` erstellte, von abgeleitete Klasse im Ordner " *Areas/Identity/Data/* ".
* *Bereiche/Identity/IdentityHostingStartup.cs*löschen.
* Aktualisieren Sie die * \_Datei "loginpartial. cshtml* ":
  * Verschieben Sie *pages\_/loginpartial. cshtml* in *pages/Shared\_/loginpartial. cshtml*.
  * Fügen `asp-area="Identity"` Sie dem Formular und den Anker Links hinzu.
  * Aktualisieren Sie `<form />` das- `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`Element auf.

  Der folgende Code zeigt die aktualisierte * \_Datei "loginpartial. cshtml* ":
...
* Ändern `Layout = "/Pages/_Layout.cshtml"` Sie `Layout = "/Pages/Shared/_Layout.cshtml"`in *Areas\_/Identity/Pages/viewstart. cshtml*in.
...
## <a name="changes-to-mvc-projects-razor-files"></a>Änderungen an MVC- Razor Projektdateien
...
<xref:Microsoft.AspNetCore.Mvc.FileResult>verarbeitet den [Accept-Ranges-](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges) Header nicht mehr standardmäßig. Um den `Accept-Ranges` -Header zu aktivieren <xref:Microsoft.AspNetCore.Mvc.FileResult.EnableRangeProcessing> , `true`legen Sie auf fest.
...
Legen Sie den `Accept-Ranges` `EnableRangeProcessing` -Parameter auf `true`fest, um den-Header zu aktivieren.
