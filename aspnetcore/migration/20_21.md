...
* Für 2,1 ist eine [Razor-Klassenbibliothek](xref:razor-pages/ui-class) die empfohlene Lösung für die Verteilung von Razor-Dateien. Wenn Ihre APP eingebettete Sichten verwendet oder anderweitig von der Runtime-Kompilierung von Razor-Dateien abhängig ist, fügen Sie `<CopyRefAssembliesToPublishDirectory>true</CopyRefAssembliesToPublishDirectory>` einer `<PropertyGroup>` in der Projektdatei hinzu.
