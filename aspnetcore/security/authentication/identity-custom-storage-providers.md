...
author: ardalis
description: Erfahren Sie, wie Sie benutzerdefinierte Speicher Anbieter Identityfür ASP.net Core konfigurieren.
ms.author: riande
ms.custom: mvc
ms.date: 07/23/2019
no-loc:
...
ASP.net Core Identity ist ein erweiterbares System, mit dem Sie einen benutzerdefinierten Speicher Anbieter erstellen und mit Ihrer APP verbinden können. In diesem Thema wird beschrieben, wie ein angepasster Speicher IdentityAnbieter für ASP.net Core erstellt wird. Es behandelt die wichtigsten Konzepte zum Erstellen eines eigenen Speicher Anbieters, aber keine schrittweise exemplarische Vorgehensweise.
...
ASP.net Core Identity besteht aus Klassen, die als Manager und Stores bezeichnet werden. *Manager* sind Klassen, die von einem App-Entwickler zum Ausführen von Vorgängen verwendet werden, z Identity . b. das Erstellen eines Benutzers. *Stores* sind Klassen auf niedrigerer Ebene, die angeben, wie Entitäten, z. b. Benutzer und Rollen, beibehalten werden. Filialen befolgen das Repository-Muster und sind eng mit dem Persistenzmechanismus verknüpft. Manager sind von Geschäften entkoppelt, d. h. Sie können den Persistenzmechanismus ersetzen, ohne den Anwendungscode zu ändern (mit Ausnahme der Konfiguration).
...
In den folgenden Abschnitten werden [ASP.net Core Identity ](https://github.com/aspnet/identity) Datentypen beschrieben:
...
Die `IdentityUser` -Klasse definiert die Eigenschaften, `UserManager` die von beim Ausführen angeforderter Vorgänge aufgerufen werden. Der Standardtyp der `Id` Eigenschaft ist eine Zeichenfolge, aber Sie können von `IdentityUser<TKey, TUserClaim, TUserRole, TUserLogin, TUserToken>` erben und einen anderen Typ angeben. Das Framework erwartet, dass die Speicher Implementierung Datentyp Konvertierungen verarbeitet.

## <a name="customize-the-user-store"></a>Anpassen des Benutzer Stores

Erstellen Sie `UserStore` eine Klasse, die die Methoden für alle Daten Vorgänge für den Benutzer bereitstellt. Diese Klasse entspricht der [&lt;userstore-tuser&gt; ](/dotnet/api/microsoft.aspnetcore.identity.entityframeworkcore.userstore-1) -Klasse. Implementieren `IUserStore<TUser>` Sie `UserStore` in ihrer Klasse und die optionalen erforderlichen Schnittstellen. Basierend auf der in der APP bereitgestellten Funktionalität wählen Sie aus, welche optionalen Schnittstellen implementiert werden sollen.
...
* **Iuseremailstore ist**  
 Die [tuser&gt; -Schnitt&lt;Stelle iuseremailstore](/dotnet/api/microsoft.aspnetcore.identity.iuseremailstore-1) definiert die Methoden, die Sie zum Speichern von Benutzer-e-Mail-Adressen implementieren Sie enthält Methoden zum erhalten und Festlegen der e-Mail-Adresse und darüber, ob die e-Mail bestätigt ist.
...
* [ASP.net Core Identity ](https://github.com/dotnet/AspNetCore/tree/master/src/Identity) &ndash; dieses Repository enthält Links zu von der Community verwalteten Speicheranbietern.
